...
from sqlalchemy import create_engine, Column, Integer, String, DateTime, Insert, update, Table, MetaData, select, func, Text as text, text as Text
...
async def delete_item():
    ...
    async with aiofiles.open(file_path, mode='r',) as file:
        ...
        filtered_list = [s for s in list_ if not re.search(pattern, s)]
...
def extract_words(task):
    words = re.findall(r'\b\w+\b\s', task)
    if words:
        return words
    else:
        return "No words found or words are not formed properly"    
...
async def add_task_to_file(add_task):
    ...
    file_path = "todo_list.txt"
    await asyncio.sleep(1)
    if not extract_words(add_task)and await check_todo_list(add_task):
        message = "Task already exist or No words found or words are not formed properly" 
        return message
    elif not await check_todo_list(add_task) and await extract_words(add_task):
        ...
        if file_:
            while num < 5000000 or num > 5000000:
                ...
                if task == '':
                    ...
                    if used_file != "Task already exist or No words found or words are not formed properly":
                        return render_template('new_todo_page.html', used_dict=files, task="success", message='Word extracted successfully and added')
                    elif  used_file == "Task already exist or No words found or words are not formed properly":
                        return render_template('new_todo_page.html', used_dict=files, task='warning', message=used_file) 
        else:
...
def complete_task():    
    ...
    if request.method == 'POST':
        file = read_file()
        for file in file_: # type: ignore
            files = file_[file] # type: ignore
